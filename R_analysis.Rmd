...
table_tad_motif <- subset(table_tad_motif, size > 100000)
...
## load repeat data----
table_tad_repeat <- read.table(file = "data/phylo_MRF_output_global_V2/rmsk/tad/v1/rmsk_hg38_align.DI.thresh10.normalize.txt", 
                               header = T,
                               sep = '\t',
                               check.names = F)
...
## use length feature----
repeat_family_col <- setdiff(grep('.len', colnames(table_tad_repeat_merge)),
                               grep('\\?', colnames(table_tad_repeat_merge)))
...
## change ratio = 0 to the minal ratio observed----
#repeat_min <- unlist(table_tad_repeat[, 4:ncol(table_tad_repeat)])
#repeat_min <- repeat_min[repeat_min!=0] 
#repeat_min <- min(repeat_min)
#table_tad_repeat[table_tad_repeat < 0.001] <- NA
...
for (col in colnames(table_tad_repeat)[4:ncol(table_tad_repeat)]) {
  ...
  if (length(group_nonconserved) > 2 & length(group_conserved) > 2) {
    pval <- wilcox.test(group_nonconserved, group_conserved)
    if (pval$p.value < 0.1 & !is.na(pval$p.value)) {
...
table_tad_repeat <- melt(table_tad_repeat, 
                        ...
                        variable.name = "repeat_family", 
                        value.name = "ratio")
table_tad_repeat$ratio <- as.numeric(table_tad_repeat$ratio)
## plot repeat ----
table_tad_repeat$group <- table_tad_repeat$serial
table_tad_repeat$log2_ratio <- log2(table_tad_repeat$ratio)
tad_repeat_pass_enrich <- subset(tad_repeat_pass, fc > log2(1.1))
tad_repeat_pass_deplete <- subset(tad_repeat_pass, fc < -log2(1.1))
plot_tad_repeat_enrich <- ggplot(subset(table_tad_repeat, repeat_family %in% tad_repeat_pass_enrich$repeat_family), 
       aes(x = repeat_family, 
           y = log2_ratio, 
           color = group)) +
  geom_hline(yintercept = 0.0) +
  ...
  my_theme +
  theme(axis.text.x = element_text(hjust = 1, angle = 45))
plot_tad_repeat_deplete <- ggplot(subset(table_tad_repeat, repeat_family %in% tad_repeat_pass_deplete$repeat_family), 
       aes(x = repeat_family, 
           y = log2_ratio, 
           color = group)) +
  geom_hline(yintercept = 0.0) +
...
C_enriched_repeat <- subset(table_tad_repeat_merge, C > 1.1)$repeat_family
N_enriched_repeat <- subset(table_tad_repeat_merge, C < 0.9)$repeat_family
plot_tad_repeat_merge <- ggplot(table_tad_repeat_merge, aes(x = C, y = N)) +
  geom_point() +
  geom_point(data = subset(table_tad_repeat_merge, C < 0.9 | C > 1.1), 
             aes(x = C, y = N), color = "red") +
  geom_text_repel(data = subset(table_tad_repeat_merge, C < 0.9 | C > 1.1), 
            aes(x = C, y = N, label = repeat_family), 
            size = 4,
            vjust = 0.5) +
            #nudge_y = - (subset(table_tad_repeat_merge, C < 0.9 | C > 1.1)$N - 1)) +
            #check_overlap = F,
            #hjust = 0, 
            #nudge_x = 0.01) +
  geom_hline(yintercept = 1.0) +
  ...
  ylab("Ratio (Non-conserved TADs)") +
  my_theme
## plot repeat (merged selected) -----
plot_tad_repeat_enrich_merge <- ggplot(subset(table_tad_repeat, repeat_family %in% N_enriched_repeat), 
       aes(x = repeat_family, 
           y = log2_ratio, 
           color = group)) +
  geom_hline(yintercept = 0.0) +
  ...
  xlab("") +
  ylab("log2 enrichment ratio") +
  ggtitle("Nonconserved enriched (merge)") +
  #coord_cartesian(ylim = c(-10, 10)) +
  my_theme +
  theme(axis.text.x = element_text(hjust = 1, angle = 45))
plot_tad_repeat_deplete_merge <- ggplot(subset(table_tad_repeat, repeat_family %in% C_enriched_repeat), 
       aes(x = repeat_family, 
           y = log2_ratio, 
           color = group)) +
  geom_hline(yintercept = 0.0) +
  ...
  xlab("") +
  ylab("log2 enrichment ratio") +
  ggtitle("Conserved enriched (merge)") +
  coord_cartesian(ylim = c(-4, 4)) +
  my_theme +
  theme(axis.text.x = element_text(hjust = 1, angle = 45))
...
dev.off()
pdf(file = 'figure/figure_global_off_diagnal_tad_repeat_merge.pdf', width= 7, height = 7)
...
dev.off()
pdf(file = "figure/figure_global_off_diagnal_tad_repeat.pdf", width = 8, height =5)
print(plot_tad_repeat_enrich)
print(plot_tad_repeat_deplete)
print(plot_tad_repeat_enrich_merge)
print(plot_tad_repeat_deplete_merge)
dev.off()
```
