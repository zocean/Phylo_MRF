...
## load motif data----
table_tad_motif <- read.table(file = "data/phylo_MRF_output_global_V2/motif/tad/percentage/motif_hg38_align.DI.thresh10.local.percentage.txt",
                              header = T,
                              sep = '\t',
                              check.names = F)
table_tad_motif_merge <- read.table(file = "data/phylo_MRF_output_global_V2/motif/motif_hg38_align.DI.thresh10.normalize.merge.local.txt",
                              ...
                              sep = '\t',
                              check.names = F)
table_tad_motif_merge_pval <- read.table(file = "data/phylo_MRF_output_global_V2/motif/tad/motif_hg38_align.DI.thresh10.chi2.txt",
                              header = F,
                              sep = '\t',
                              check.names = F)
table_tad_motif_merge_pval <- cbind(table_tad_motif_merge[, 1:4], table_tad_motif_merge_pval)
colnames(table_tad_motif_merge_pval) <- colnames(table_tad_motif_merge)
...
table_tad_motif_merge <- table_tad_motif_merge[, colnames(table_tad_motif_merge)[colSums(table_tad_motif_merge, na.rm = T) > 0]]
table_tad_motif_merge_pval <- table_tad_motif_merge_pval[, colnames(table_tad_motif_merge)[colSums(table_tad_motif_merge, na.rm = T) > 0]]
## intersect with motif annotation-----
motif_anno <- read.table(file = "data/phylo_MRF_output_global_V2/motif/motif_names_1.txt",
                         header = F,
                         sep = '\t')
...
rownames(table_tad_motif_merge_pval) <- rownames(table_tad_motif_merge)
# transpose the data.frame
table_tad_motif_merge <- as.data.frame(t(table_tad_motif_merge[, 4:ncol(table_tad_motif_merge)]))
...
table_tad_motif_merge_pval <- as.data.frame(t(table_tad_motif_merge_pval[, 4:ncol(table_tad_motif_merge_pval)]))
table_tad_motif_merge_pval$motif_id <- rownames(table_tad_motif_merge_pval)
table_tad_motif_merge <- merge(table_tad_motif_merge, table_tad_motif_merge_pval,
                               by = "motif_id", suffixes = c(".ratio", ".pval"))
table_tad_motif_merge$log10_pval_C <- -log10(table_tad_motif_merge$C.pval + 2.2e-16)
# select motif id
selected_motif <- subset(table_tad_motif_merge, log10_pval_C > 2 & (C.ratio < 0.85 | C.ratio > 1.15))
plot_tad_motif_merge <- ggplot(table_tad_motif_merge, aes(x = C.ratio, y = log10_pval_C)) +
  geom_point(alpha = 0.8, pch = 21, fill = "grey") +
  geom_point(data = subset(table_tad_motif_merge, log10_pval_C > 2 & (C.ratio < 0.85 | C.ratio > 1.15)),  
             aes(x = C.ratio, y = log10_pval_C), color = "red") +
  geom_text_repel(data = subset(table_tad_motif_merge, log10_pval_C > 2 & (C.ratio < 0.85 | C.ratio > 1.15)), 
            aes(x = C.ratio, y = log10_pval_C, label = tf), 
            size = 4,
            vjust = 0.5) +
  geom_hline(yintercept = 0.0) +
  geom_hline(yintercept = 2.0, linetype = 2, color = "darkgrey") +
  geom_vline(xintercept = 1.0) +
  coord_cartesian(ylim = c(0, 17), xlim = c(0.4, 1.6)) +
...
for (col in selected_motif$motif_id) {
  group_nonconserved <- table_tad_motif[which(table_tad_motif$serial == 'N'), col]
  group_conserved <- table_tad_motif[which(table_tad_motif$serial == 'C'), col]
...
table_tad_motif <- melt(table_tad_motif, 
                        ...
                        variable.name = "motif_id", 
                        value.name = "per")
table_tad_motif$per <- as.numeric(table_tad_motif$per)
## plot motif ----
...
table_tad_motif_select <- subset(table_tad_motif, motif_id %in% selected_motif$motif_id)
table_tad_motif_select$motif_id <- factor(table_tad_motif_select$motif_id, 
                                          levels = selected_motif$motif_id)
#
stat <- tapply(table_tad_motif_select$per, 
               list(table_tad_motif_select$motif_id, table_tad_motif_select$group),
               function(x) boxplot.stats(x))
stats <- unlist(tapply(table_tad_motif_select$per, 
               list(table_tad_motif_select$motif_id, table_tad_motif_select$group),
               function(x) boxplot.stats(x))$stats)
table_tad_motif_select_boxplot <-data.frame(
  motif_id = rep(rep(unlist(dimnames(stat)[1]), each= 5),
                    length(unlist(dimnames(stat)[2]))),
  group = rep(unlist(dimnames(stat)[2]), each = 5*length(selected_motif$motif_id)),
  per = unlist(tapply(table_tad_motif_select$per,list(table_tad_motif_select$motif_id,table_tad_motif_select$group),function(x) boxplot.stats(x)$stats)))
...
table_tad_motif_merge$motif_name <- paste0(table_tad_motif_merge$tf,
                                 ...
                                 table_tad_motif_merge$motif_id)
#
table_tad_motif_select_boxplot$tf <- motif_anno$tf[match(table_tad_motif_select_boxplot$motif_id, motif_anno$motif_id)]
table_tad_motif_select_boxplot$motif_name = paste0(table_tad_motif_select_boxplot$tf, 
                                                   '_',
                                                   table_tad_motif_select_boxplot$motif_id)
#
plot_tad_motif_enrich <- ggplot(table_tad_motif_select_boxplot, 
       aes(x = group, 
           y = per, 
           fill = group)) +
  geom_hline(yintercept = 0.0) +
  ...
  facet_wrap(~ motif_name, ncol = 6, scales = "free_y") +
  xlab("") +
  ylab("Percentage") +
...
# transpose the data.frame
table_tad_repeat_merge <- as.data.frame(t(table_tad_repeat_merge[, 4:ncol(table_tad_repeat_merge)]))
table_tad_repeat_merge$repeat_family <- rownames(table_tad_repeat_merge)
table_tad_repeat_merge_pval <- as.data.frame(t(table_tad_repeat_merge_pval[, 4:ncol(table_tad_repeat_merge_pval)]))
...
plot_tad_repeat_merge <- ggplot(table_tad_repeat_merge, aes(x = C.ratio, y = log10_pval_C)) +
  ...
  geom_hline(yintercept = 2.0, linetype = 2, color = "darkgrey") +
  geom_vline(xintercept = 1.0) +
  coord_cartesian(ylim = c(0, 17), xlim = c(0.7, 1.3)) +
  xlab("Ratio (Conserved TADs)") +
  ylab("-log10(P value)") +
  my_theme
## add group -----
...
pdf(file = 'figure/figure_global_off_diagnal_tad_motif_merge.pdf', width = 6, height = 4)
print(plot_tad_motif_merge)
dev.off()
pdf(file = "figure/figure_global_off_diagnal_tad_motif.pdf", width = 12, height = 6)
