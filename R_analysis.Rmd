...
```{r}
# load data----
table_tad_repeat <- read.table(file = "data/phylo_MRF_output_global_V2/rmsk/tad/rmsk_hg38_align.DI.thresh10.normalize.txt", 
                               header = T,
                               sep = '\t',
                               check.names = F)
table_tad_repeat_merge <- read.table(file = "data/phylo_MRF_output_global_V2/rmsk/tad/rmsk_hg38_align.DI.thresh10.normalize.merge.txt", 
                               header = T,
                               sep = '\t',
                               check.names = F)
table_tad_motif <- read.table(file = "data/phylo_MRF_output_global_V2/motif/motif_hg38_align.Arrowhead.thresh10.normalize.local.txt",
                              header = T,
                              sep = '\t',
                              check.names = F)
...
table_tad_motif <- table_tad_motif[, colnames(table_tad_motif_merge)[colSums(table_tad_motif_merge) > 0]]
...
tad_motif_pass_fc = c()
for (col in colnames(table_tad_motif)[5:ncol(table_tad_motif)]) {
  group_nonconserved <- log2(table_tad_motif[which(table_tad_motif$serial == 0), col])
  group_conserved <- log2(table_tad_motif[which(table_tad_motif$serial == 1), col])
  ...
  if (length(group_nonconserved) > 2 & length(group_conserved) > 2) {
    ...
    if (pval$p.value < 0.05) {
      ...
      tad_motif_pass_fc <- c(tad_motif_pass_fc, mean(group_nonconserved, na.rm =T) - mean(group_conserved, na.rm = T))
    }
  }
}
...
table_tad_motif <- melt(table_tad_motif, 
                        id.vars = c("chrom", "start", "stop", "serial"), 
                        variable.name = "motif_id", 
                        value.name = "ratio")
## intersect with motif annotation-----
motif_anno <- read.table(file = "data/phylo_MRF_output_global_V2/motif/motif_names_1.txt",
...
table_tad_motif$group <- ifelse(table_tad_motif$serial == 0, 'N', 'C')
table_tad_motif$log2_ratio <- log2(table_tad_motif$ratio)
tad_motif_pass_enrich <- subset(tad_motif_pass, fc > log2(1.15))
tad_motif_pass_deplete <- subset(tad_motif_pass, fc < - log2(1.1))
...
for (col in colnames(table_tad_repeat)[5:ncol(table_tad_repeat)]) {
  group_nonconserved <- log2(table_tad_repeat[which(table_tad_repeat$serial == 0), col])
  group_conserved <- log2(table_tad_repeat[which(table_tad_repeat$serial == 1), col])
  group_nonconserved <- group_nonconserved[!is.na(group_nonconserved) & is.finite(group_nonconserved)]
  group_conserved <- group_conserved[!is.na(group_conserved) & is.finite(group_conserved)]
  if (length(group_nonconserved) > 2 & length(group_conserved) > 2) {
    pval <- wilcox.test(group_nonconserved, group_conserved)
    if (pval$p.value < 0.05 & !is.na(pval$p.value)) {
...
table_tad_repeat <- melt(table_tad_repeat, 
                        ...
                        value.name = "ratio")
## plot repeat ----
table_tad_repeat$group <- ifelse(table_tad_repeat$serial == 0, 'N', 'C')
table_tad_repeat$log2_ratio <- log2(table_tad_repeat$ratio)
tad_repeat_pass_enrich <- subset(tad_repeat_pass, fc > log2(1.15))
tad_repeat_pass_deplete <- subset(tad_repeat_pass, fc < -log2(1.15))
